    ...
    <div className="page-container antialiased pb-20 bg-[#fbfcfd] min-h-screen">
      <div className="max-w-[1600px] mx-auto px-4 space-y-6">
        <div className="flex items-center justify-between gap-4">
          <h1 className="text-3xl font-black text-heading-slate">{t(packagesTitle)}</h1>
          <button onClick={() => setShowCreate(true)} className="btn-emerald px-6 py-3 rounded-2xl">{t("Add New Package")}</button>
        </div>
      ...
      {deleteRow && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-slate-900/60 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md">
            <p className="text-sm text-slate-700 mb-4">{t("Delete")} "{deleteRow.name || deleteRow.id}"?</p>
